...
door is locked, but it includes a small interface. "Restricted Area - Strictly No Digitized
Users Allowed."
...
def calculate_captcha(data, dist_to_next_digit):
    """
    Calculates the solution to the provided input captcha data.
    """

    return sum(int(data[i]) for i in range(len(data)) \
        if data[i] == data[(i + dist_to_next_digit) % len(data)])
...
if __name__ == '__main__':
    ...
    print(calculate_captcha(DATA, 1))
    print(calculate_captcha(DATA, len(DATA) // 2))
